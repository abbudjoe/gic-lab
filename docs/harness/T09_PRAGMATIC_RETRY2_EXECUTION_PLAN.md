...
- 2026-08-13: independent review found incomplete realized evaluator-overlay/package
  closure, a missing gross-ceiling checkpoint branch, a missing sensitive account-key
  spelling, and an unrecoverable pre-entry condition prefix. Each root control now has
  a focused adversarial regression; no provider, model, or browser action occurred.
- 2026-08-13: rereview identified the repository-external dotenv selector as a
  previously unbound security helper. Its exact ancestor byte (`6cd57c…`) is now in
  the runtime identity, the focused suite proves exact-one assignment selection,
  unsafe-input/mode rejection, and O_EXCL mode-0600 output, and the live invocation
  is permitted only while the provider lifecycle's exact clean-package check remains
  current. No credential value has been read.
- 2026-08-13: froze the repaired V4 runtime source at reviewed ancestor
