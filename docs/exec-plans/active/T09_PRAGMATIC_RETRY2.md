...
This plan implements the user-supplied Retry 2 contract at
`/Users/joseph/.codex/attachments/35b71d85-2abd-4e1c-895e-ed13e8697e24/pasted-text.txt`
...
|---|---|---|---|
| R2-01 | Preserve the V3 disposition, archive, provider receipts, image observation, and all V3/T07 identities byte-for-byte. | Historical-hash regressions and separate V4 paths/IDs. | not-started |
| R2-02 | Freeze exactly the existing science: EXP-0001, two task IDs/hashes, FanOutQA revision, SiRA commit, GPT-4o snapshot, evaluator, counterbalance, zero retry, calibration-only interpretation. | V3/V4 science-lock comparison and schema tests. | not-started |
| R2-03 | Build the candidate only from pinned inputs and retain base, Containerfile, complete context, source, dependency, browser, evaluator, entrypoint, environment, build-command, Docker/BuildKit, and SOURCE_DATE_EPOCH evidence. | Replacement build manifest and source-hash validation. | not-started |
| R2-04 | Replace historical image-ID equality with a typed acceptance rule: one accepted replacement image, exact ID bound before empirical entry, and unchanged image verified for every attempt. | Static V4 policy plus mode-0600 dynamic runtime binding and per-attempt inspect check. | not-started |
| R2-05 | Compare available T07 image evidence and classify each observed difference without treating digest inequality alone as functional. | Image-equivalence adjudication. | not-started |
| R2-06 | Pass final-container functional equivalence for Python 3.11.14, SiRA/source, workdir/entrypoint, packages/imports/evidence/budgets/tasks, exact offline evaluator fixtures, local browser lifecycle, command/pair diffs, cleanup, and exactly one model metadata request with no task request/action. | Source-bound qualification receipt and retained raw evidence. | not-started |
| R2-07 | Freeze V4 plan, host/condition/evaluator/archive IDs ending in 0002, four commands/configs, code, runtime binding, and manifest before the first condition empirical event. | Clean source package, private runtime overlay, freeze receipt, and git/plan hashes. | not-started |
| R2-08 | Preserve a verified loadable image archive when it does not threaten the campaign/cleanup wall; otherwise record the exact limitation. | OCI/Docker archive identity or deferred-export receipt. | not-started |
| R2-09 | Enforce one 14,400-second provider clock, 13,500-second termination cutoff, 900-second cleanup reserve, next-attempt-only admission, one instance/launch, and zero persistent filesystem. | Fake-clock tests plus source-bound provider receipts. | not-started |
| R2-10 | Enforce fresh and cumulative cost limits, 4,620 calls, 4,000,000 tokens, 120 browser actions, four attempts, and zero scientific retry before each billable action. | Static/dynamic ledgers, runtime counters, first-pair projection, compute reconciliation. | not-started |
...
| R2-13 | Retain reconstructable, structurally redacted private evidence for every entered attempt and the maximal safe failure prefix. | Per-attempt exports, score/evidence schema validation, archive manifest/hashes. | not-started |
| R2-14 | Prioritize cleanup: owned containers/browsers absent, temporary secrets destroyed, evidence copied as available, exact instance terminated/absent, firewall restored, regional rulesets absent, no T09 instance running. | Host cleanup plus source-bound closeout receipts. | not-started |
| R2-15 | Pass focused image/science/pair/timing/budget/evidence/cleanup tests, `make validate`, privacy scan, `git diff --check`, and independent spec-conformance review before launch; repair and rereview any material finding. | Command logs and reviewer verdict. | not-started |
