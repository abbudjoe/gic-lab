...
MAX_PREFLIGHT_LAMBDA_COST_USD: Final = 10.0
...
IMAGE_ARCHIVE_PATH: Final = Path("/home/ubuntu/t09-pilot-v7-replacement-image-0001.tar")
...
ATTEMPT_EXPORT_REQUIRED_CONTROL_PATHS: Final = (
    "pilot-v7/provider-entry.json",
    "pilot-v7/frozen-run-manifest.json",
    "pilot-v7/postfreeze-validation.json",
    "pilot-v7/model-metadata-credential-scan.json",
    "pilot-v7/core-suppression-preflight/host-core-suppression.json",
    "pilot-v7/final-image-file-hashes/receipt.json",
    "pilot-v7/qualified-real-evidence-regression/receipt.json",
    "pilot-v7/local-finalizer-qualification.json",
...
def _elf_object_type(path: Path) -> str | None:
    """Read only the bounded ELF identification header."""
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode):
            raise T09HostError("core scan encountered an unsafe file identity")
    ...
    byte_order = header[5]
    if byte_order == 1:
        object_type = int.from_bytes(header[16:18], "little")
    elif byte_order == 2:
        object_type = int.from_bytes(header[16:18], "big")
    else:
        return "ELF_UNKNOWN"
    return {
        1: "ET_REL",
        ...
        3: "ET_DYN",
        4: "ET_CORE",
    }.get(object_type, f"ET_{object_type}")


def detect_core_artifacts(root: Path) -> list[dict[str, object]]:
    """Identify known core names and ELF ET_CORE using bounded metadata reads."""

    resolved = root.resolve(strict=True)
    if not resolved.is_dir() or resolved.is_symlink():
        raise T09HostError("core scan root is unsafe")
    scanned: list[tuple[dict[str, object], tuple[int, int] | None]] = []
    entry_count = 0
    for path in sorted(resolved.rglob("*")):
        entry_count += 1
        if entry_count > MAX_CORE_SCAN_ENTRIES:
            raise T09HostError("core scan entry limit exceeded")
        metadata = path.lstat()
        if stat.S_ISDIR(metadata.st_mode):
            continue
        indicators: list[str] = []
        if _CORE_FILENAME.fullmatch(path.name):
            indicators.append("known-core-filename")
        file_type = (
            "symlink"
        ...
        elf_type = _elf_object_type(path) if file_type == "regular" else None
        if elf_type == "ET_CORE":
            indicators.append("elf-et-core")
        record: dict[str, object] = {
            ...
            "classified_links_within_scan_root": 0,
        ...
        identity = (metadata.st_dev, metadata.st_ino) if file_type == "regular" else None
        scanned.append((record, identity))
    core_identities = {
        identity
        for record, identity in scanned
        if identity is not None and cast(list[object], record["indicators"])
    }
    in_root_link_counts: dict[tuple[int, int], int] = {}
    for _record, identity in scanned:
        if identity in core_identities:
            assert identity is not None
            in_root_link_counts[identity] = in_root_link_counts.get(identity, 0) + 1
    records: list[dict[str, object]] = []
    for record, identity in scanned:
        indicators = cast(list[str], record["indicators"])
        if identity in core_identities and not indicators:
        ...
        record["destruction_verifiable"] = (
            identity is not None and type(link_count) is int and link_count == classified_links
        )
        records.append(record)
    return records


def remove_core_artifacts(root: Path, records: list[dict[str, object]]) -> bool:
    """Remove classified paths and report whether every inode link was owned."""
    ...
    for record in records:
        ...
        if (
            ...
            raise T09HostError("core cleanup received an unsafe relative path")
        ...
        if (
            metadata.st_size != record.get("bytes")
            ...
            raise T09HostError("core artifact changed before security cleanup")
        if (
            file_type == "regular"
            and "elf-et-core" in cast(list[object], record.get("indicators"))
            and _elf_object_type(target) != "ET_CORE"
        ):
            raise T09HostError("core ELF identity changed before security cleanup")
    ...
    for parent in parents:
        ...
        try:
            ...
            os.close(directory)
    if detect_core_artifacts(resolved):
        raise T09HostError("core artifact destruction could not be verified")
...
def _zero_inheritance_projection(value: object, *, expected_depth: int) -> bool:
    if not isinstance(value, dict):
        return False
    if (
        value.get("depth") != expected_depth
        or value.get("core_soft_limit") != 0
        or value.get("core_hard_limit") != 0
    ):
        return False
    ...
    *,
    ...
    create = [
        ...
        *CORE_ULIMIT_DOCKER_ARGS,
        ...
        "--mount",
        f"type=bind,src={source},dst=/opt/giclab/t09_core_preflight.py,readonly",
        "--mount",
        (
            "type=bind,src="
            f"{repository / 'containers/sira-smoke/container_entrypoint.py'},"
    ...
    if (
        ...
        or receipt.get("core_hard_limit") != 0
...
def _prior_qualification_failure_manifest(root: Path) -> dict[str, object]:
    if root.name != "t09-pilot-v7-output-0002":
    ...
    env_example = (
        root / "pilot-v7/replacement-image-qualification/work/build-context/upstream/.env.example"
    )
    if (
        ...
        or (root / "pilot-v7/frozen-run-manifest.json").exists()
        or (root / "pilot-v7/replacement-image-qualification/build-context-manifest.json").exists()
    ):
...
def _failed_candidate_manifest(
    ...
    *,
    ...
    if root.name != "t09-pilot-v7-output-0003":
        raise T09HostError("failed candidate root identity drifted")
    state = load_object(root / "pilot-v7/pilot-state.json", label="failed candidate state")
    ...
    stderr_path = root / "pilot-v7/offline-runtime-preflight/offline-preflight.stderr"
    provider_entry = load_object(
        root / "pilot-v7/provider-entry.json", label="prior provider entry"
    )
    ...
    if (
        ...
        or (root / "pilot-v7/frozen-run-manifest.json").exists()
        or (root / "pilot-v7/preflight.json").exists()
        or (root / "pilot-v7/model-metadata-preflight").exists()
        or (root / "pilot-v7/browser-preflight").exists()
    ):
...
def provider_seconds_remaining(root: Path, *, reserve_seconds: float = 0.0) -> float:
    ...
    if (
        ...
        remaining = min(
            ...
            (MAX_EMPIRICAL_LAMBDA_COST_USD - empirical_elapsed * LAMBDA_HOURLY_PRICE_USD / 3600.0)
            * 3600.0
    ...
    *,
    ...
    create = [
        ...
        *CORE_ULIMIT_DOCKER_ARGS,
        ...
        (
            "type=bind,src="
            f"{Path(__file__).resolve(strict=True).parents[1] / 'bounded/browser_preflight.py'},"
            "dst=/opt/giclab/browser_preflight.py,readonly"
        ),
        "--entrypoint",
        "/opt/sira/.venv/bin/python",
        image_id,
...
def qualified_real_evidence_regression(
    *,
    ...
    if (
        receipt.get("receipt_id") != "T09-PRAGMATIC-RETRY5-QUALIFIED-IMAGE-REGRESSION-0001"
        or receipt.get("source_archive_sha256")
        != "63ed19b35bcb4cb62c3796a80a48004937340eb3826f9657a1006e251772255d"
        or receipt.get("dataset_sha256")
        != "359300b029c6891567816f351bf8786e9b018d7af8a1a44b7da9ba5ef4651288"
...
def provider_accounting_container_preflight(
    *,
    repository: Path,
    artifact_root: Path,
    prefix: list[str],
    ...
    command = [
        ...
        *CORE_ULIMIT_DOCKER_ARGS,
        ...
        f"type=bind,src={attempt},dst=/giclab/attempt",
        "--mount",
        f"type=bind,src={repository / 'src'},dst=/opt/giclab-accounting-src,readonly",
        "--mount",
    ...
    accounting = receipt.get("accounting_snapshot")
    terminal_counts = accounting.get("terminal_counts") if isinstance(accounting, dict) else None
    if (
        receipt.get("result") != "passed"
        or receipt.get("live_model_requests") != 0
        or receipt.get("task_browser_actions") != 0
        or receipt.get("thirty_response_three_provider_error") != "passed"
        or receipt.get("concurrent_atomic_reservation") != "passed"
        or receipt.get("bounded_shutdown_unknown_terminalization") != "passed"
        or not isinstance(accounting, dict)
        or accounting.get("unknown_outcomes") != 0
        or accounting.get("unreconciled_provider_attempts") != 0
        or accounting.get("outstanding_reservations") != 0
        or terminal_counts
        != {
            "admitted_not_sent": 0,
        ...
        raise T09HostError("exact-container provider accounting regression failed")
    return {**receipt, "receipt_sha256": file_sha256(receipt_path)}
    ...
    *,
    ...
    command = [
        ...
        *CORE_ULIMIT_DOCKER_ARGS,
        ...
        (
            "type=bind,src="
            f"{Path(__file__).resolve(strict=True).parents[1] / 'container_entrypoint.py'},"
            "dst=/opt/giclab/container_entrypoint.py,readonly"
        ),
        "--entrypoint",
        "/opt/sira/.venv/bin/python",
...
def write_frozen_run_manifest(
    *,
    ...
    if empirical != [] or runtime_state.get("nonempirical_infrastructure_attempts_consumed") != []:
    ...
    if (
        ...
        or runtime_state.get("core_safety_stop_detected") is not False
    ...
    if launch_slot == 1:
        ...
        if (
            ...
            or slot2_authority.get("replacement_eligibility_preempirical_source_manifest_sha256")
    ...
    manifest: dict[str, object] = {
        ...
        "sealing_primitives_preflight_sha256": canonical_sha256(sealing_primitives_receipt),
        ...
        **resume_fields,
        "source_receipts": {
            ...
            "slot2_authority_tree": dynamic.get("normalized_slot2_authority_tree_manifest_sha256"),
    ...
    *,
    ...
    if typed_qualification.launch_slot == 1:
        ...
        if (
            ...
            != typed_qualification.replacement_eligibility_sha256
            or authority.get("replacement_eligibility_preempirical_source_manifest_sha256")
            != typed_qualification.replacement_eligibility_preempirical_source_manifest_sha256
    ...
    if (
        ...
        or sealing_preflight.get("essential_failure_cap_bytes") != MAX_ESSENTIAL_FAILURE_BYTES
        ...
        or core_preflight.get("synthetic_abort_nonzero") is not True
        or core_preflight.get("core_filename_count") != 0
        or core_preflight.get("elf_et_core_count") != 0
        or core_preflight.get("container_removed") is not True
        or core_preflight.get("owned_container_residue") != []
...
def prepare_preflight_resume(
    ...
    if (
        prior_root.name != "t09-pilot-v7-preflight-failure-0001"
...
def resume_preflight(args: argparse.Namespace) -> None:
    ...
    if (
        ...
        or lambda_elapsed * LAMBDA_HOURLY_PRICE_USD / 3600.0 >= MAX_PREFLIGHT_LAMBDA_COST_USD
    ...
    write_exclusive(
        preflight_path,
        {
            ...
            "provider_call_accounting": provider_accounting,
...
def preflight(args: argparse.Namespace) -> None:
    ...
    if launch_slot == 1:
        ...
        if (
            slot2_authority.get("replacement_eligibility_sha256")
            != dynamic.get("replacement_eligibility_sha256")
            or slot2_authority.get("replacement_eligibility_preempirical_source_manifest_sha256")
            != dynamic.get("replacement_eligibility_preempirical_source_manifest_sha256")
            or slot2_authority.get("normalized_slot2_authority_tree_manifest_sha256")
...
def preflight_with_deadline(args: argparse.Namespace) -> None:
    ...
    if not all(
        ...
        raise T09HostError("provider entry lacks Retry 5 clock accounting")
    ...
    deadline = min(
        ...
        (MAX_PREFLIGHT_LAMBDA_COST_USD - active_cost) * 3600.0 / LAMBDA_HOURLY_PRICE_USD,
    )
    if deadline <= 0:
...
def _limit_file_size() -> None:
    resource.setrlimit(
        ...
        (MAX_ATTEMPT_STREAM_BYTES, MAX_ATTEMPT_STREAM_BYTES),
    )
    ...
    with stdout_path.open("xb") as stdout, stderr_path.open("xb") as stderr:
        ...
        while process.poll() is None:
            ...
            if elapsed > MAX_CONDITION_WALL_SECONDS:
                ...
                stop_reason = "condition_stdout_bytes"
                hard_cap_breached = True
            elif os.fstat(stderr.fileno()).st_size >= MAX_ATTEMPT_STREAM_BYTES:
                stop_reason = "condition_stderr_bytes"
                hard_cap_breached = True
        ...
        if stop_reason is None and stdout_bytes >= MAX_ATTEMPT_STREAM_BYTES:
            stop_reason = "condition_stdout_bytes"
            hard_cap_breached = True
        if stop_reason is None and stderr_bytes >= MAX_ATTEMPT_STREAM_BYTES:
            stop_reason = "condition_stderr_bytes"
            hard_cap_breached = True
        if stop_reason is None and returncode in {-signal.SIGXFSZ, 128 + signal.SIGXFSZ}:
    ...
    *,
    ...
    if attempt_root.exists() or attempt_root.is_symlink():
        ...
        if (
            ...
            or retained.get("core_safety_stop_detected") is not False
    ...
    *,
    ...
    write_exclusive(
        attempt_root / "preentry-condition-failure.json",
        {
            ...
            "core_safety_stop_detected": core_safety_stop_detected,
            "remaining_exact_secret_matches": remaining,
    ...
    if (
        exposure_detected
        or credential_cleanup_integrity_failure
...
class ConditionRunnerExceptionDisposition(NamedTuple):
    """Truthful terminal authority for a host attach/control exception."""
...
def condition_runner_exception_disposition(
    error: Exception,
) -> ConditionRunnerExceptionDisposition:
    """Classify an attach/control exception without fabricating a byte-cap breach."""

    return ConditionRunnerExceptionDisposition(
        returncode=125,
...
def record_output_cap_event(
    *,
    ...
    """Publish a cap event only when a byte/stream cap was actually reached."""
    ...
    write_exclusive(
        path,
        {
            "schema_version": "0.1.0",
            "run_id": run_id,
            "attempt_output_hard_cap_bytes": MAX_ATTEMPT_OUTPUT_BYTES,
            "attempt_tree_bytes_before_security_cleanup": (tree_bytes_before_security_cleanup),
            "attempt_tree_bytes_after_core_cleanup": tree_bytes_after_core_cleanup,
            "stop_reason": stop_reason,
            "hard_cap_breached": True,
            "core_artifact_count": len(core_records),
            "core_artifact_bytes": sum(cast(int, item["bytes"]) for item in core_records),
            "full_raw_seal_permitted": False,
...
def _is_essential_failure_candidate(relative: str) -> bool:
    if relative in _ESSENTIAL_FAILURE_EXACT_PATHS:
        return True
    path = PurePosixPath(relative)
    return (
        len(path.parts) >= 2
        and path.parts[0] == "sira-output"
        and path.suffix.lower() in _ESSENTIAL_FAILURE_NESTED_SUFFIXES
    )


def _copy_failure_evidence_file(source: Path, destination: Path) -> dict[str, object]:
        ...
        try:
            while chunk := os.read(source_descriptor, 1_048_576):
                ...
                if copied > metadata.st_size:
                    raise T09HostError("essential-failure source grew during copy")
                digest.update(chunk)
                offset = 0
                while offset < len(chunk):
                    written = os.write(destination_descriptor, chunk[offset:])
                    if written <= 0:
                        raise T09HostError("essential-failure copy made no progress")
                    offset += written
            os.fsync(destination_descriptor)
        finally:
            os.close(destination_descriptor)
        if copied != metadata.st_size or destination.stat().st_size != metadata.st_size:
            raise T09HostError("essential-failure copy size changed")
    finally:
...
def _essential_failure_files(root: Path) -> tuple[list[dict[str, object]], int]:
    files: list[dict[str, object]] = []
    total = 0
    for path in sorted(root.rglob("*")):
        metadata = path.lstat()
        ...
        relative = path.relative_to(root).as_posix()
        if path.is_symlink() or not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
            raise T09HostError("essential-failure bundle contains an unsafe member")
        total += metadata.st_size
...
def seal_essential_failure(
    *,
    ...
    run_id = cast(str, manifest["run_id"])
    task_index = RUN_IDS.index(run_id) // 2
    retained_state = load_object(
        artifact_root / "pilot-v7/pilot-state.json",
        label="essential-failure pilot state",
    )
    entered = retained_state.get("empirical_attempts_entered")
    nonempirical = retained_state.get("nonempirical_infrastructure_attempts_consumed")
    if (
        not isinstance(entered, list)
        or not isinstance(nonempirical, list)
        or (run_id in entered) != empirical_entry_crossed
    ...
    for source in sorted(raw_root.rglob("*")) if raw_tree_scan_safe else ():
        ...
        if category is not None:
            safe_digest = (
                file_sha256(source)
                if category == "not-in-essential-failure-allowlist"
    ...
    failure_summary = {
        ...
        "task_row_sha256": TASK_ROW_SHA256S[task_index],
        "task_text_sha256": TASK_TEXT_SHA256S[task_index],
        "task_reference_sha256": TASK_REFERENCE_SHA256S[task_index],
...
def validate_essential_failure_seal(
    *,
    ...
    if (
        ...
        raise T09HostError("essential-failure seal no longer reconstructs from exact bytes")
    ...
    *,
    ...
    write_exclusive(
        supervisor / "runtime-reconstruction-binding.json",
        {
            ...
            "host_cleanup_receipt_sha256": file_sha256(supervisor / "host-cleanup-receipt.json"),
    ...
    *,
    ...
    try:
        ...
        raise T09HostError(
            ...
            "infrastructure-invalid essential failure"
        ) from exc
...
def validate_postfreeze_entry_receipts(
    *,
    ...
    if (
        ...
        or preflight_receipt.get("raw_attempt_sealing", {}).get("raw_reconstruction_passed")
        is not True
        or preflight_receipt.get("raw_attempt_sealing", {}).get("essential_reconstruction_passed")
        is not True
...
def execute_condition(args: argparse.Namespace) -> int:
    ...
    try:
        try:
            ...
            disposition = condition_runner_exception_disposition(exc)
            returncode = disposition.returncode
            ...
            infrastructure_stop_requires_essential_seal = (
                disposition.infrastructure_stop_requires_essential_seal
            )
    finally:
        try:
    ...
    seal_consumed_raw_or_essential_failure(
        artifact_root=artifact_root,
        attempt_root=attempt_root,
        raw_root=raw_root,
        manifest=manifest,
...
def _specialize_finalized_identity_schemas(
    *,
    ...
    identity = (
        evidence_properties.get("identity") if isinstance(evidence_properties, dict) else None
    )
    evidence_runtime = (
...
def reject_and_remove_finalized_core_artifacts(
    *,
    artifact_root: Path,
    finalized_root: Path,
    run_id: str,
    ...
    raise T09HostError("finalizer produced a prohibited core artifact; campaign stopped")
...
def finalize_attempt(args: argparse.Namespace) -> dict[str, object]:
    ...
    if raw_manifest_path.is_file() and not essential_manifest_path.exists():
        raw_manifest, raw_receipt = validate_raw_attempt_seal(
            attempt_root=attempt_root,
            raw_root=raw_root,
            run_id=args.run_id,
    ...
    elif essential_manifest_path.is_file() and not raw_manifest_path.exists():
        validate_essential_failure_seal(
            attempt_root=attempt_root,
            run_id=args.run_id,
            package_commit=args.package_commit,
...
def _attempt_export_control_sources(
    ...
    *,
    ...
    if evidence_authority == "immutable-raw-attempt":
        ...
        seals = state.get("essential_failure_seals")
        binding = seals.get(run_id) if isinstance(seals, dict) else None
        failure_receipt = load_object(
            attempt_root / "essential-failure-complete.json",
            label="export essential-failure receipt",
        )
        empirical_entry_crossed = failure_receipt.get("empirical_entry_crossed")
        entered = state.get("empirical_attempts_entered")
        nonempirical = state.get("nonempirical_infrastructure_attempts_consumed")
        if (
            ...
            raise T09HostError("attempt export control state lacks its essential-failure seal")
    else:
        raise T09HostError("attempt export evidence authority is unsupported")
...
def export_attempt(args: argparse.Namespace, destination: BinaryIO) -> dict[str, object]:
    ...
    essential_manifest_path = attempt_root / "essential-failure-manifest.json"
    if raw_manifest_path.is_file() and not essential_manifest_path.exists():
        seal_manifest, seal_receipt = validate_raw_attempt_seal(
            attempt_root=attempt_root,
            raw_root=raw_root,
            run_id=args.run_id,
            package_commit=args.package_commit,
        ...
        archive_prefix = "raw"
        seal_names = ("raw-attempt-manifest.json", "raw-attempt-complete.json")
        downstream_finalization_required = True
    elif essential_manifest_path.is_file() and not raw_manifest_path.exists():
        seal_manifest, seal_receipt = validate_essential_failure_seal(
            attempt_root=attempt_root,
            run_id=args.run_id,
            package_commit=args.package_commit,
            condition_manifest=manifest,
        ...
        archive_prefix = "essential-failure"
        seal_names = ("essential-failure-manifest.json", "essential-failure-complete.json")
    ...
    sources: dict[str, Path] = {}
    total = 0
    sealed_files = seal_manifest.get("files")
    if not isinstance(sealed_files, list):
        raise T09HostError("attempt evidence manifest file set is malformed")
    for raw in cast(list[dict[str, object]], sealed_files):
        relative = cast(str, raw["path"])
        size = cast(int, raw["bytes"])
        total += size
        archive_name = f"{archive_prefix}/{relative}"
        sources[archive_name] = evidence_root / relative
    ...
    export_manifest: dict[str, object] = {
        ...
        "empirical_entry_crossed": (
            True
            if evidence_authority == "immutable-raw-attempt"
            else seal_receipt.get("empirical_entry_crossed")
        ),
        "downstream_finalization_required_for_analysis": downstream_finalization_required,
        "files": files,
        "total_bytes": total,
        ...
        "campaign_continuation_permitted": (
            evidence_authority == "immutable-raw-attempt"
            and seal_receipt.get("credential_cleanup_clean") is True
        ),
        "structural_privacy_scan_passed": (
            seal_receipt.get("public_release_clearance") is True
            if evidence_authority == "immutable-raw-attempt"
            else True
        ),
...
def verify_attempt_export(args: argparse.Namespace) -> None:
    ...
    with tarfile.open(archive, "r:gz") as handle:
        ...
        if (
            ...
            or manifest.get("frozen_run_manifest_id") != FROZEN_RUN_MANIFEST_ID
            ...
            or manifest.get("evidence_authority")
        ...
        evidence_authority = manifest.get("evidence_authority")
        if evidence_authority == "immutable-raw-attempt":
            ...
            try:
                raw_receipt: object = (
                    json.loads(raw_receipt_stream.read())
                    if raw_receipt_stream is not None
                    else None
                )
            except json.JSONDecodeError as exc:
            ...
            if (
                ...
                or manifest.get("downstream_finalization_required_for_analysis") is not True
            ):
                raise T09HostError("attempt export privacy adjudication is not source-bound")
        else:
            failure_receipt = archive_json(
                "essential-failure-complete.json",
                label="essential-failure receipt",
            )
            failure_summary = archive_json(
                "essential-failure/failure-summary.json",
                label="essential-failure summary",
            )
            if (
                failure_receipt.get("essential_failure_seal_complete") is not True
                or failure_receipt.get("attempt_identity_consumed") is not True
                or failure_receipt.get("empirical_entry_crossed")
                != manifest.get("empirical_entry_crossed")
                or failure_summary.get("empirical_entry_crossed")
                != manifest.get("empirical_entry_crossed")
                or failure_receipt.get("infrastructure_invalid") is not True
                ...
                or failure_summary.get("condition_retry_permitted") is not False
                or manifest.get("campaign_continuation_permitted") is not False
                or manifest.get("downstream_finalization_required_for_analysis") is not False
            ):
                raise T09HostError("essential-failure export is not source-bound")
...
def restore_verified_attempt_export(
    *,
    ...
    with tarfile.open(archive, "r:gz") as handle:
        ...
        if (
            evidence_authority not in {"immutable-raw-attempt", "essential-infrastructure-failure"}
            or not isinstance(empirical_entry_crossed, bool)
            or (
                evidence_authority == "immutable-raw-attempt"
                and empirical_entry_crossed is not True
            )
        ):
            raise T09HostError("offline restoration evidence authority is unsupported")
        ...
        for record in file_records:
            ...
            elif name.startswith("essential-failure/"):
    ...
    expected_entered = list(
        RUN_IDS[: expected_index + 1] if empirical_entry_crossed else RUN_IDS[:expected_index]
    )
    expected_raw = (
        expected_entered
        if evidence_authority == "immutable-raw-attempt"
        else list(RUN_IDS[:expected_index])
    )
    ...
    if (
        ...
        or nonempirical_consumed != ([] if empirical_entry_crossed else [run_id])
    ...
    if state_path.exists():
        ...
        if retained_raw_tuple == tuple(expected_raw) and retained != snapshot_state:
    ...
    restore_receipt = {
        ...
        "evidence_authority": evidence_authority,
        "empirical_entry_crossed": empirical_entry_crossed,
        "qualified_local_finalization_ready": (evidence_authority == "immutable-raw-attempt"),
        "essential_failure_reconstructable": (
            evidence_authority == "essential-infrastructure-failure"
        ),
    }
...
def acknowledge_attempt_export(args: argparse.Namespace) -> None:
    ...
    raw_complete = state.get("raw_attempts_complete")
    essential_seals = state.get("essential_failure_seals")
    raw_authority = isinstance(raw_complete, list) and args.run_id in raw_complete
    essential_authority = isinstance(essential_seals, dict) and isinstance(
        essential_seals.get(args.run_id), dict
    )
    if raw_authority == essential_authority:
        raise T09HostError(
            "cannot acknowledge an attempt without one exact evidence-seal authority"
        )
    expected_index = RUN_IDS.index(args.run_id)
    assert isinstance(raw_complete, list)
    expected_prior_raw = list(RUN_IDS[:expected_index])
    expected_with_current = list(RUN_IDS[: expected_index + 1])
    if (raw_authority and raw_complete[: expected_index + 1] != expected_with_current) or (
        essential_authority and raw_complete != expected_prior_raw
    ):
...
def _reconstructable_disposition(
    ...
    *,
    ...
    if checkpoint == "continue-to-task-b":
        ...
        if (
            not isinstance(checkpoint_binding, dict)
            or checkpoint_document.get("decision") != checkpoint
            or state.get("first_pair_decision_sha256") != canonical_sha256(checkpoint_document)
            or checkpoint_binding.get("decision_sha256") != canonical_sha256(checkpoint_document)
        ):
            raise T09HostError("Task B failure lacks its exact first-pair checkpoint authority")
    if (
        not isinstance(entered, list)
        or not isinstance(nonempirical_consumed, list)
        ...
        or (
            bool(nonempirical_consumed)
            and (len(entered) >= len(RUN_IDS) or nonempirical_consumed != [RUN_IDS[len(entered)]])
        )
    ...
    pending = entered[len(raw_complete) :]
    if pending and nonempirical_consumed:
        raise T09HostError("empirical and non-empirical failure authorities overlap")
    if pending:
        failed_run_id = pending[0]
        failed_index = RUN_IDS.index(failed_run_id)
        binding = essential_seals.get(failed_run_id)
        if (
            not isinstance(binding, dict)
            or set(essential_seals) != {failed_run_id}
            or completed != raw_complete
            or (failed_index < 2 and checkpoint == "continue-to-task-b")
            or (failed_index >= 2 and checkpoint != "continue-to-task-b")
        ):
            raise T09HostError(
                "consumed unscored attempt lacks one terminal essential-failure seal"
            )
    elif nonempirical_consumed:
        failed_run_id = nonempirical_consumed[0]
        failed_index = RUN_IDS.index(failed_run_id)
        binding = essential_seals.get(failed_run_id)
        if (
            not isinstance(binding, dict)
            ...
            or completed != entered
            or set(finalizations) != set(completed)
            or (failed_index < 2 and checkpoint == "continue-to-task-b")
            or (failed_index >= 2 and checkpoint != "continue-to-task-b")
        ):
            raise T09HostError(
                "non-empirical consumed attempt lacks one terminal essential-failure seal"
            )
...
def cleanup(args: argparse.Namespace) -> None:
    ...
    if cleanup_core_records or historic_global_records or effective_global_scan_failure:
        ...
        if not isinstance(execution_for_core, str):
            raise T09HostError("cleanup cannot bind a detected core to pilot state")
        mark_core_safety_stop(
            state_path,
...
def _preempirical_late_gate_absence(root: Path) -> dict[str, object]:
    ...
    present = [
        ...
        if os.path.lexists(pilot_root / relative)
    ]
    if present:
        raise T09HostError("pre-empirical failure crossed the model-metadata/freeze boundary")
    return {
        "schema_version": "0.1.0",
        "plan_id": PLAN_ID,
        "host_run_id": HOST_RUN_ID,
        "checked_relative_paths": list(_PREEMPIRICAL_LATE_GATE_PATHS),
...
def preempirical_replacement_disposition(args: argparse.Namespace) -> dict[str, object]:
    ...
    write_exclusive(source / "late-preflight-gate-absence.json", late_gate_absence)
    receipt: dict[str, object] = {
        "schema_version": "0.1.0",
        "plan_id": PLAN_ID,
        "host_run_id": HOST_RUN_ID,
        ...
        "late_preflight_gate_absence_sha256": file_sha256(
